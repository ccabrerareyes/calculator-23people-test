...
const useCalculator = () => {
    ...
    function clickOperator(value){
        var number = parseFloat(state.counter)
        ...
        if(!isNaN(number)){
            if(value === "="){
                ...
                if(state.firstNumber === null && state.secondNumber === null){
                    setState( state => ({ ...state,firstNumber: number, operator: value, isNewNumber: true}) )
    ...
    function inputnumber(value){
        if(state.isNewNumber){
            if(value === "."){
                if(parseFloat(state.counter) === 0 || !String(state.counter).includes(".")){
                    setState( state => ({ ...state, counter: String(0) + value, isNewNumber: false}) )
                }
            }else{
            ...
            switch (value) {
                case ".":
                    if(parseFloat(state.counter) === 0 || !String(state.counter).includes(".")){
                        setState( state => ({ ...state, counter: String(state.counter) + value}) )
                    }
                    break;
                default:
                    if(parseFloat(state.counter) !== 0 || String(state.counter).includes(".")){
    ...
    function calculate(){
        switch (state.operator) {
            case "+":
                setState( state => ({ ...state,counter: parseFloat(state.firstNumber) + parseFloat(state.secondNumber)}) )
                break;
            case "-":
                setState( state => ({ ...state,counter: parseFloat(state.firstNumber) - parseFloat(state.secondNumber)}) )
                break;
            ...
            case "/":
                ...
                break;
            case "=":
                setState( state => ({ ...state,counter: parseFloat(state.secondNumber) }) )
                break;
            default:
                break;
        }
